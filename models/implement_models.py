...
def train_SVM(train_data: list | np.ndarray, targets: list, grid: bool) -> svm.LinearSVC:
    ...
    if grid:
        ...
        # default hyperparameters for tailored features: {max_iter=1000, regularizer(C)=0.001}
        # default hyperparameters for ngram features: {max_iter=1000, regularizer(C)=10}
...
def train_RF(train_data: list | np.ndarray, targets: list, grid: bool) -> ensemble.RandomForestClassifier:
    ...
    if grid:
        print('Using grid search to find the best parameter combination.')
        print('Paramter options: {"max_depth":[30, 50], "max_features":("sqrt", "log2", None), "n_estimators":[100]}')
        parameters = {'max_depth':[30, 50], 'max_features':('sqrt', 'log2', None)}
        rf =  ensemble.RandomForestClassifier(n_estimators=100)
        rf_clf = GridSearchCV(rf, parameters, scoring='accuracy', n_jobs=-1, verbose=2)
